...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const numX1 = Math.sqrt(x1 * x1 + y1 * y1);
  const numX2 = Math.sqrt(x2 * x2 + y2 * y2);
  const res = num / (numX1 * numX2);
  const all = Math.acos(res);
  return all;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const num = Math.sqrt(a * a + b * b + c * c);
  return num;
}
...
function roundToPowerOfTen(num, pow) {
  const number = 10 ** pow;
  const res = Math.round(num / number) * number;
  return res;
}
